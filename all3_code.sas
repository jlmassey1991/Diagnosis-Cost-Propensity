...
*Creating Total Hospital Days and Outpatient Visits;
data delay5;
set delay4;
hosp_days_total = hosp_before_days + hosp_after_days;
op_visits_total = op_before_num + op_after_num ;
run;

*Check distributions of Total Hospital Days and Outpatient Visits; 
proc sgplot data = delay5;
histogram hosp_days_total;
run;

proc sgplot data = delay5;
histogram op_visits_total;
run;

proc freq data = delay5;
tables op_after_num;
run;

*Creating quantile groups to include in table 1 ;
...
proc freq data = delay4; 
tables op_before_num
op_after_num
hosp_before_days
hosp_after_days ;
run;
...
data tables;
set delay4;
op_days_total = op_before_num + op_after_num ;
op_total_cost = op_before_cost + op_after_cost ;

if hosp_before or hosp_after = 1 then hosp_total = 1;
else hosp_total = 0 ;
hosp_days_total = hosp_before_days + hosp_after_days ;
hosp_total_cost = hosp_before_cost + hosp_after_cost ;
run;

data table3; 
set tables;
if op_days_total = 0 then op_total = 0 ;
else op_total = 1 ;
run; 
...
%macro pathogen(pathogen);

*No. Outpatient;
proc freq data = table3;
tables op_total ;
where disease_type = &pathogen. ;
run;

*OP Mean Cost;
proc means data = table3;
var op_total_cost;
where op_total = 1 and disease_type = &pathogen. ;
run;
...
proc means data = table3;
var hosp_total_cost;
where hosp_total = 1 and disease_type = &pathogen. ;
run;
...
*Mean Antibiotic Cost;
proc means data = table3;
var antibiotics_cost;
where antibiotics = 1 and disease_type = &pathogen. ;
run;

*Mean Antifungal Cost;
proc means data = table3;
var antifungals_cost;
where antifungals = 1 and disease_type = &pathogen. ;
run;
