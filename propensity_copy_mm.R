...
survival <- read.csv("//cdc.gov/project/CCID_NCPDCID_NHSN_SAS/Data/work/_Projects/LTC/COVID-19/Codes/Jason/Mary_Projects/SVH_survival/propensity_updatedmm.csv")
...
# Read in Facility Covariate Data
urbanicity <- read.csv("//cdc.gov/project/CCID_NCPDCID_NHSN_SAS/Data/work/_Projects/LTC/COVID-19/Codes/Jason/Mary_Projects/SVH_survival/Jason Urbanicity.csv")

# Left Join Covariates to propensity 
propensity <- left_join(survival, covariates, by = "orgid")

# Left Join Urbanicity to propensity 
propensity1 <- left_join(propensity, urbanicity, by = "orgid")
...
propensity$facowner <- ifelse(propensity$facowner== "VA", "GOV", propensity$facowner )

# Combine NP and GOV to Non-Private 
propensity$facowner <- ifelse(propensity$facowner == "GOV", "NP", propensity$facowner )
...
table(propensity$biv, propensity$status)
# Drop Missing

propensity2 <- propensity[ which(propensity$urbanicity != ""), ] 
table(propensity2$biv, propensity2$status)
# Drop Territory 
propensity3 <- propensity2[which(propensity2$region != "Territory"), ] 
table(propensity3$biv, propensity3$status)

propensity <- propensity3 

# Set References for each Covariate 
propensity$region <- relevel(factor(propensity$region), "Midwest" )
propensity$urbanicity <- relevel(factor(propensity$urbanicity), "Urban")
propensity$ltcCert <- relevel(factor(propensity$ltcAff), "Ind")
propensity$facowner <-  relevel(factor(propensity$facowner), "NP")
...
propensity_s = subset(propensity, select = c(status, days, biv, age, race, ltcAff,
                                           urbanicity, region ))
...
iptw.model=glm(biv~ age + as.factor(ltcAff) + as.factor(urbanicity) + as.factor(region),
               data=propensity,
...
new.propensity <-augment(iptw.model,
  ...
  select(status,days, biv, age, ltcAff, urbanicity, region, propensity)

head(new.propensity,6) 
